...
def edit_myeducation(request):
    ...
    if request.method == "POST":
        ...
        schoolgpg = request.POST['schoolgpg']
...
def emp_salary(request):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    employee = EmployeeDetail.objects.all()
    return render(request, 'emp_salary.html', locals())


def edit_salary(request, pid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    error = ""
    ...
    if request.method == "POST":
        ...
        try:
            ...
            error = "yes"
    return render(request, 'edit_salary.html', locals())
